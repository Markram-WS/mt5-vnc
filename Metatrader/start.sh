...
export MT5_AUTO_START_EA="${MT5_AUTO_START_EA:-TestAutoStartEA}"
export MT5_AUTO_START_SYMBOL="${MT5_AUTO_START_SYMBOL:-EURUSD}"
export MT5_AUTO_START_PERIOD="${MT5_AUTO_START_PERIOD:-H1}"
...
for _host in update.mql5.com updates.mql5.com; do
    if ! grep -q "$_host" /etc/hosts 2>/dev/null; then
        if printf '127.0.0.1 %s\n' "$_host" >> /etc/hosts 2>/dev/null; then
            echo "[start.sh] Blocked MT5 LiveUpdate ($_host) in /etc/hosts"
        else
            echo "[start.sh] WARN: cannot write /etc/hosts (need root) - LiveUpdate NOT blocked ($_host)"
        fi
    fi
done
...
# ---- MT5 launch + EA auto-start ----

terminal_dir="${WINEPREFIX}/drive_c/users/root/AppData/Roaming/MetaQuotes/Terminal"
...
# Launch MT5. EA auto-start is handled by startup.ini (written in
# enable_auto_start_ea), which build 6090 reads to attach the EA on boot.
# Historical auto-attach approaches (inject_common_ini + /config: cli file) were
# removed because build 6090 ignored them and logged "cannot load config" each boot.
launch_mt5() {
    if [ "$has_credentials" = true ]; then
        ...
        $wine_executable "$mt5file" /login:"$MT5_ACCOUNT" /password:"$MT5_PASSWORD" /server:"$MT5_SERVER" /portable &
    else
...
find_terminal_uuid_dir() {
    find "$terminal_dir" -maxdepth 1 -type d \( -name '????????-????-????-????-????????????' -o -name '????????????????????????????????' \) ! -name 'Common' ! -name 'Community' 2>/dev/null | head -1
}

# Write startup.ini BEFORE MT5 reads it so the EA is auto-attached to a chart
# (Expert + Symbol + Period opens the chart and attaches the EA on launch).
# On a fresh volume the terminal UUID directory only exists after MT5 has run
# once, so start MT5 briefly, write startup.ini, then relaunch it once.
enable_auto_start_ea() {
    local ea_name="${MT5_AUTO_START_EA:-TestAutoStartEA}"
    local symbol="${MT5_AUTO_START_SYMBOL:-EURUSD}"
    local period="${MT5_AUTO_START_PERIOD:-H1}"
    local uuid_dir
    local launched_first=false

    uuid_dir=$(find_terminal_uuid_dir)

    if [ -z "$uuid_dir" ]; then
        show_message "[start.sh] [7/7] Fresh volume: starting MT5 briefly to create terminal data dir..."
        launch_mt5
        launched_first=true
        local waited=0
        while [ -z "$uuid_dir" ] && [ $waited -lt 60 ]; do
            sleep 2
            waited=$((waited + 2))
            uuid_dir=$(find_terminal_uuid_dir)
        done
        if [ -z "$uuid_dir" ]; then
            show_message "[start.sh] [7/7] ERROR: terminal data dir not found after ${waited}s; EA auto-start skipped"
            return 1
        fi
        show_message "[start.sh] [7/7] Terminal data dir created: $uuid_dir"
    fi

    # Write startup.ini to BOTH locations:
    #  - install root (portable-mode data dir): MT5 reads this at startup (and
    #    consumes it).  When bind-mounted from the host this also refreshes the
    #    mounted file so a one-shot deletion can't leave it missing.
    #  - terminal UUID dir under AppData: kept by MT5 but often ignored.
    local mt5_dir="${WINEPREFIX}/drive_c/Program Files/MetaTrader 5"
    local startup_ini="${uuid_dir}/startup.ini"
    local startup_ini_mt5="${mt5_dir}/startup.ini"
    cat > "$startup_ini" <<EOF
[StartUp]
Expert=${ea_name}
Symbol=${symbol}
Period=${period}
EOF
    cp "$startup_ini" "$startup_ini_mt5"
    show_message "[start.sh] [7/7] Auto-start EA configured: ${ea_name} on ${symbol} ${period} -> ${startup_ini} + ${startup_ini_mt5}"

    if [ "$launched_first" = true ]; then
        show_message "[start.sh] [7/7] Relaunching MT5 so it reads startup.ini..."
...
# Auto-start the configured EA (must run before the xdotool login loop below)
enable_auto_start_ea

# Ensure MT5 is running. enable_auto_start_ea only launches MT5 on a fresh volume
# (where it is started briefly to create the profile, then relaunched); on an
# existing volume we must launch it here.
if [ "$mt5_started" != true ]; then
    show_message "[start.sh] [7/7] Launching MT5..."
    launch_mt5
fi
