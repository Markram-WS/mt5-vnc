...
export WINEPREFIX=/app
export WINEDLLOVERRIDES="winemenubuilder.exe=d,mscoree=n"
export XDG_RUNTIME_DIR="${XDG_RUNTIME_DIR:-/tmp}"
export XDG_DATA_HOME="${XDG_DATA_HOME:-/app/.local/share}"

# Fix: Wine defaults to ~/.wine (/config/.wine for abc user) and ignores WINEPREFIX
# when the prefix doesn't exist yet. Symlink ensures /config/.wine points to /app.
if [ ! -e /config/.wine ]; then
    ln -s /app /config/.wine
fi
...
if ! grep -q "update.mql5.com" /etc/hosts 2>/dev/null; then
    if printf '127.0.0.1 update.mql5.com\n' >> /etc/hosts 2>/dev/null; then
...
# Ensure WINEPREFIX is owned by the runtime user (container runs as abc, not root)
# Without this, Wine falls back to ~/.wine (/config/.wine) because /app is root-owned
mkdir -p "$WINEPREFIX"
